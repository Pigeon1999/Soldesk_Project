...
@Getter
@ToString
public class AdminPagingCreatorDTO {

	private AdminPagingDTO adminPaging ;
	...
	public AdminPagingCreatorDTO(long rowTotal,
								AdminPagingDTO adminPaging,
		...
		//계산된 끝 페이징 번호:
		this.endPagingNum = (int) Math.ceil((double) adminPaging.getPageNum()/this.pagingNumCnt) * this.pagingNumCnt ;

		//계산된 시작 페이징 번호:
		this.startPagingNum = this.endPagingNum - (this.pagingNumCnt -1) ;

		//총 페이지 수 = 맨 마지막 페이지번호
		this.lastPageNum = (int) Math.ceil((double)this.rowTotal / this.adminPaging.getRowAmountPerPage()) ;
		...
		System.out.println("전달된 페이징 기본데이터-adminPagingDTO: " + this.adminPaging.toString());
		...
		System.out.println("표시할 데이터: " + this.userList);
	}
